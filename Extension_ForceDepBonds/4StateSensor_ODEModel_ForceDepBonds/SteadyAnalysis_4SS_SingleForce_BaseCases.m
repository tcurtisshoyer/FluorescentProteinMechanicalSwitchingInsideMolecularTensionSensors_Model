...
SweepPropsUnbinding.bondModelID = [1 2 3]; % Specifies which bond model to use
SweepPropsUnbinding.intrinsicRateConstantScaling = [1 1 1]; % Specifies scaling of koff
...
for cc=1:length(Sweep_F)
    ...
    if SweepPropsUnbinding.bondModelID(bb)==1
        BondType = {'Ideal'};
    elseif SweepPropsUnbinding.bondModelID(bb)==2
        BondType = {'Slip'};
    elseif SweepPropsUnbinding.bondModelID(bb)==3
        BondType = {'CatchSlip'};
    end
...
%% [1] Acceptor Only: Ideal, Slip, CatchSlip

% SimulationSetID_toPlot=1:length(unique(Data.SimulationSetID)); % All
SimulationSetID_toPlot = [4 5 6];
...
for SimulationSetID=SimulationSetID_toPlot
    ...
    legend_text = [legend_text, [thisData.BondType{1} ' Bond']];
...
% Save full figure panel in specified file formats
FigName = 'UnbindingRateConstants_BaseCases_Acceptor';
Resolution_png = 300;
print(f,fullfile(PlotOutDir,[FigName '.png']),'-dpng',['-r' num2str(Resolution_png)],'-painters');
print(f,fullfile(PlotOutDir,[FigName '.svg']),'-dsvg','-painters');

%% [2] Donor Only: Ideal, Slip, CatchSlip

% SimulationSetID_toPlot=1:length(unique(Data.SimulationSetID)); % All
SimulationSetID_toPlot = [7 8 9];
...
% Save full figure panel in specified file formats
FigName = 'UnbindingRateConstants_BaseCases_Donor';
Resolution_png = 300;
print(f,fullfile(PlotOutDir,[FigName '.png']),'-dpng',['-r' num2str(Resolution_png)],'-painters');
print(f,fullfile(PlotOutDir,[FigName '.svg']),'-dsvg','-painters');
