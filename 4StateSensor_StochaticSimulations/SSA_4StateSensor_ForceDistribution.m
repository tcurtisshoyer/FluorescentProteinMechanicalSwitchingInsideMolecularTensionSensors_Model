...
% Rate Constants for Kinetic Model
...
SensorUnbindingRateConstantDistribution = ModelParams.SensorUnbindingRateConstantDistribution;
...
%% Initial Conditions
AllSensors_Force = SensorForceDistribution;
AllSensors_UnbindRateConstant = SensorUnbindingRateConstantDistribution;
...
while t<tend
    ...
    % Col 2: State Variable ID:
        % State_Donor = 1
        % State_Acceptor = 2
        % NOTE: Turnover Transition (D#A#-->D1A1) indicated using 0
    ...
    for thisSensorID = 1:numSensors
        F = AllSensors_Force(thisSensorID);
        ...
        rate = AllSensors_UnbindRateConstant(thisSensorID);
        thisPossibleTransition = [thisSensorID thisStateVariableID 0 0 rate F];
        allTransitions(count+1,:) = thisPossibleTransition;
        count = count + 1;
    ...
    % Reset Time Since Last Turnover of Given Sensor if Turnover Event Occured
