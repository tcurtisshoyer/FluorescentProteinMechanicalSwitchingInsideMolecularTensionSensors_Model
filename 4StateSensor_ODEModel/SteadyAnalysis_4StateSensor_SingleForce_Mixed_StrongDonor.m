...
SweepPropsFPs.F_half_D = [5]*ones(1,8);
SweepPropsFPs.m_D = [1.5]*ones(1,8);
SweepPropsFPs.kms0_D = [1]*ones(1,8);
SweepPropsFPs.dxms_D = [.23]*ones(1,8);

% Mech Switch Rate Constant for Acceptor
% kmsA = kms(F,F_half_A,m_A,kms0_A,dxms_A);
F_half_sweep = [5 8 15 25];
m_sweep = [1.5 1 1 1];
SweepPropsFPs.F_half_A = [5 F_half_sweep 5 5 5];
SweepPropsFPs.m_A = [1.5 m_sweep 1.5 1.5 1.5];
SweepPropsFPs.kms0_A = [0 1 1 1 1 .1 .316 1];
SweepPropsFPs.dxms_A = [.23 .23 .23 .23 .23 .23 .23 .23];
...
SimulationSetID_toPlot = [2:5 1]; % Sweep F_half_D
...
FigName = 'ES_StrongDonor_SweepFhalfA';
Resolution_png = 300;
...
% Save full figure panel in specified file formats
FigName = 'RateConstants_StrongDonor_SweepFhalfA';
Resolution_png = 300;
print(f,fullfile(PlotOutDir,[FigName '.png']),'-dpng',['-r' num2str(Resolution_png)],'-painters');
print(f,fullfile(PlotOutDir,[FigName '.svg']),'-dsvg','-painters');
...
% SimulationSetID_toPlot=1:length(unique(Data.SimulationSetID)); % All
SimulationSetID_toPlot = [6:8 1]; % Sweep kms0_D
...
% Save full figure panel in specified file formats
FigName = 'ES_StrongDonor_Sweepkms0A';
Resolution_png = 300;
print(f,fullfile(PlotOutDir,[FigName '.png']),'-dpng',['-r' num2str(Resolution_png)],'-painters');
print(f,fullfile(PlotOutDir,[FigName '.svg']),'-dsvg','-painters');
...
% Save full figure panel in specified file formats
FigName = 'RateConstants_StrongDonor_Sweepkms0A';
Resolution_png = 300;
print(f,fullfile(PlotOutDir,[FigName '.png']),'-dpng',['-r' num2str(Resolution_png)],'-painters');
print(f,fullfile(PlotOutDir,[FigName '.svg']),'-dsvg','-painters');
